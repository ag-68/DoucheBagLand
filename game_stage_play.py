...
def run_game_stage_play():
    ...
    if server_communicate == True:
        ...
        init_player_info = submodules.split_init_info(net.init_info)
        ...
        client_player = Player(client_player_ID, inits.nameButton.name, posInfo, icon_info[0], icon_info[1],
                               ...
                               inits.playerLog["KickWait"], inits.playerLog["PunchDamage"], inits.playerLog["KickDamage"])
        ...
        while run:
            ...
            if client_player_info.health <= 0:
                ...
                if not gameX:
                    ...
                    #playerInfoList = gameX.playerInfoList
                    ...
                    for info in playerInfoList:
                        if info.id != client_player_ID:
                            ...
                            op_hitInfo = [info.hitDisp, info.hit_img_orientation, info.hit_img_center, info.hitList, info.hitRotAngle, info.hitType]
                            other_player = Player(info.id, info.name, op_posInfo, icon_info[0], info.orientation, icon_num,
                    ...
                    client_player.move(settings.MOVE_TIME_WAIT, settings.WIN_DIM, settings.ICON_DIM[0], settings.ZERO_THR)

                    # check if player gets any potion
                    client_rect = client_player.getPlayerRect()
                    for potion in gameX.potionList:
                    ...
                    for player in players:
                        ...
                        if check_id != client_player_ID:
                            ...
                            if hit_info[0] == "YES":
                                ...
                                if hit_success == 1:
                                    inits.game_sound_mixer.Channel(2).play(inits.game_sound_mixer.Sound(settings.hit_success_music))
                    ...
                    drawings.draw_stage_play(players, client_player_ID, topScorer, hitLoadButton, gameX.potionList, player_condition)
                    hitType = "NONE"
